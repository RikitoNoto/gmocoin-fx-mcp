...
def _map_change_ifdoco_order(order) -> dict[str, str | int | float | None]:
    return {
...
def register_change_ifdoco_order_tools(
    mcp: FastMCP,
    api_key: str,
    secret_key: str,
) -> None:
    ...
    def change_ifdoco_order_api(
        ...
        return [_map_change_ifdoco_order(order) for order in response.orders]
